...
volatile float Fs = 6500.0; // Sample frecuency

// --- Position PID values
volatile float pKp = 15.0;
volatile float pKi = 0.2;
volatile float pKd = 250.0;
volatile float pLPF = 30.0;

// --- Velocity PID values
volatile float vKp = 0.001;
...
const int steps_per_revolution = 200;                // 200 steps per revolution  -- for 400 step/rev, you should only need to edit this value
const float angle_per_step = 360.0 / steps_per_revolution;       // angle per step
int count_per_revolution = 16384;                    // counts per rev
const float stepangle = angle_per_step / 32.0;   // for step/dir interrupt: aps/32 is the equivalent of 1/32 microsteps

volatile float PA = angle_per_step;
